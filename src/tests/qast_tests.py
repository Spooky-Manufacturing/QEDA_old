...
class QAST_Tests(unittest.TestCase):
    ...
    def test_cq_gate(self):
        control = 1
        targets = [2,3,4,5]
        self.assertEqual(self.cq_gate(control, targets).eval(),
                         (control, targets, None))

    def test_custom_q_gate(self):
        self.assertEqual(self.custom_q_gate().eval(),
                         (None, None, None))
        control = 1
        targets = [1,2,3,4]
        self.assertEqual(self.custom_q_gate(control, targets).eval(),
                         (control, targets, None))

    def test_h_gate(self):
        self.assertEqual(self.h(0).eval(),
                         (qast.LIB, qast.config[qast.CSIZE]['HGate']))

    def test_i_gate(self):
        self.assertEqual(self.i(0).eval(),
                         (qast.LIB, qast.config[qast.CSIZE]['IGate']))

    def test_s_gate(self):
        self.assertEqual(self.s(0).eval(),
                         (qast.LIB, qast.config[qast.CSIZE]['SGate']))

    def test_sdg_gate(self):
        self.assertEqual(self.sdg(0).eval(),
                         (qast.LIB, qast.config[qast.CSIZE]['SDGGate']))

    def test_t_gate(self):
        self.assertEqual(self.t(0).eval(),
                         (qast.LIB, qast.config[qast.CSIZE]['TGate']))

    def test_tdg_gate(self):
        self.assertEqual(self.tdg(0).eval(),
                         (qast.LIB, qast.config[qast.CSIZE]['TDGGate']))

    def test_x_gate(self):
        self.assertEqual(self.x(0).eval(),
                         (qast.LIB, qast.config[qast.CSIZE]['XGate']))

    def test_y_gate(self):
        self.assertEqual(self.y(0).eval(),
                         (qast.LIB, qast.config[qast.CSIZE]['YGate']))

    def test_z_gate(self):
        self.assertEqual(self.z(0).eval(),
                         (qast.LIB, qast.config[qast.CSIZE]['ZGate']))


    def test_rx_gate(self):
        self.assertEqual(self.rx(0).eval(),
                         (qast.LIB, qast.config[qast.CSIZE]['RXGate']))


    def test_rz_gate(self):
        self.assertEqual(self.rz(0).eval(),
                         (qast.LIB, qast.config[qast.CSIZE]['RZGate']))
    ...
    def test_cx_gate(self):
        ...
        self.assertEqual(self.cx(control, target).eval(),
                         (control, target,
                          (qast.LIB, qast.config[qast.CSIZE]['CXGate'])))
    ...
    def test_cy_gate(self):
        ...
        self.assertEqual(self.cy(control, target).eval(),
                         (control, target,
                          (qast.LIB, qast.config[qast.CSIZE]['CYGate'])))
    ...
    def test_cz_gate(self):
        ...
        self.assertEqual(self.cz(control, target).eval(),
                         (control, target,
                          (qast.LIB, qast.config[qast.CSIZE]['CZGate'])))
    ...
    def test_ch_gate(self):
        ...
        self.assertEqual(self.ch(control, target).eval(),
                         (control, target,
                          (qast.LIB, qast.config[qast.CSIZE]['CHGate'])))
    ...
    def test_ccx_gate(self):
        ...
        self.assertEqual(self.ccx(control, targets).eval(),
                         (control, targets,
                          (qast.LIB, qast.config[qast.CSIZE]['CCXGate'])))
    ...
    def test_crz_gate(self):
        ...
        self.assertEqual(self.crz(control, target).eval(),
                         (control, target,
                          (qast.LIB, qast.config[qast.CSIZE]['CRZGate'])))
    ...
    def test_cu1_gate(self):
        ...
        self.assertEqual(self.cu1(control, target).eval(),
                         (control, target,
                          (qast.LIB, qast.config[qast.CSIZE]['CU1Gate'])))
    ...
    def test_cu3_gate(self):
        ...
        self.assertEqual(self.cu3(control, target).eval(),
                         (control, target,
                          (qast.LIB, qast.config[qast.CSIZE]['CU3Gate'])))

    def test_measurement(self):
        self.assertEqual(self.measure(0).eval(),
                         (qast.LIB, qast.config[qast.CSIZE]['MEASURE']))
